...
■ 주소 (나만 알고 쓰세요)
  ...
  ⚠️ master.html 을 더블클릭해서 여는 것은 이제 안 됩니다 (2026-09-11 잠금 이후).
    브라우저가 file:// 에서는 목록 파일(tools.enc)을 읽지 못하게 막기 때문입니다.
    내 PC에서 보시려면 이 폴더에서 아래를 실행한 뒤
        python -m http.server
    http://localhost:8000/master.html 로 들어오세요.
    ※ 더블클릭으로 열면 화면이 이유를 알려 줍니다 (예전에는 "암호가 맞지 않습니다"
      라고만 떠서 암호를 계속 다시 치게 됐습니다 — 2026-09-17 수정).
...
■ 사용 순서
  ...
  4) 전달 방법 2가지
     📱 [링크 + QR 만들기]  ← 추천
        · 주소 하나 + QR 이미지가 나옵니다.
        · 학생은 QR을 스캔한 뒤 이번 주 코드(8자리)를 넣으면 열려요. 따로 배포할 필요 없음.
        · 고른 도구 목록이 주소 안에 암호화돼 담깁니다(전체 목록 노출 없음).
        · 도구가 많으면 주소가 길어져 QR이 빽빽해져요 → 8개 이하 권장.
     📄 [공유용 HTML 파일로 내보내기]
        · 파일 1개 다운로드 → 카톡/메일 첨부. 인터넷 없이도 열림.
        · 받는 사람은 이번 주 코드나 암호를 넣어야 목록이 보입니다.

  🔒 잠금 (2026-09-11~)
     · 받은 사람은 이번 주 코드나 교사용 암호를 "직접" 넣어야 열립니다.
       보낸 사람 기기에 암호가 기억돼 있어도 받은 사람 기기로는 넘어가지 않습니다.
     · 한 번 넣은 기기는 기억합니다. 주간 코드는 그 주가 지나면 다시 묻습니다.
     · 예전에 나눠 준 링크(?d=)와 선생님용 admin/user 링크는 잠그지 않았습니다.
     · "잠금용 공유 키가 없어요" 가 뜨면 python build_lock.py 를 다시 돌리면 됩니다.
